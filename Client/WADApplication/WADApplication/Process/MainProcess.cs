...
namespace WADApplication.Process
{
    class MainProcess
    {
        ...
        public static void addPoint(List<Equipment> list, int selectedEqId, TextEdit t1,TextEdit t2,TextEdit t3,TextEdit t4, ChartControl chart, DateTime dt)
        {
            if (chart.Series == null || list == null)
            {
                return;
            }

            Equipment one = list.Find(ii => ii.ID == selectedEqId);
            // 如果是当前选择的曲线
            if (one != null)
            {
                t1.Text = one.Chroma.ToString();
                float d2;
            ...
            {
                var ed = list.Find(im => im.ID == Convert.ToInt32(series.Tag));
                if (ed != null && series.View is SwiftPlotSeriesViewBase)
                {
                    Trace.WriteLine(ed.ID);
                    series.Points.Add(new SeriesPoint(dt, ed.Chroma));
                }
            }
        }
        ...
        public static void ManageSeries(ChartControl chartControl, Equipment sone, DateTime minTime, DateTime maxTime)
        {            
            ...
            if (sone != null)
            {
                Series series = new Series(sone.GasName, ViewType.SwiftPlot);
                series.Tag = sone.ID;
                ...
                //List<EquipmentData> datalist = EquipmentDataBusiness.GetList(minTime, maxTime, sone.ID, 2);
            ...
            if (sone != null)
            {
                diagram_Tem.AxisY.Title.Text = string.Format("浓度({0})", sone.UnitName);
            }
            else
            {
